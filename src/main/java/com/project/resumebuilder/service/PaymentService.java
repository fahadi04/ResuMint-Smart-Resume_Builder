...
public class PaymentService {
    ...
    public Payment createOrder(Object principal, String planType) throws RazorpayException {
        ...
        Payment newPayment = Payment.builder().userId(authResponse.getId()).razorpayOrderId(razorpayOrder.get("id")).amount(amount).currency(currency).planType(planType).status("created").receipt(receipt).build();
    ...
    public boolean verifyPayment(String razorpayOrderId, String razorpayPaymentId, String razorpaySignature) throws RazorpayException {
        try {
            ...
            log.info("Error verifying the payment: ", e);
            return false;
        }
    }
